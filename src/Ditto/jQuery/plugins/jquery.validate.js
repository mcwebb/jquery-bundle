        ...
        if (options['live']) {
            ...
            if (jQuery(this).find('input').length > 0) {
                ...
                jQuery(this).find('input').bind('blur', function(){
                    if (validate_field(this, options)) {
        ...
        function validate_field(e){
          ...
          // var self = jQuery(id);
           // var expression = 'function Validate(){' + options['expression'].replace(/VAL/g, 'jQuery(\'#' + self + '\').val()') + '} Validate()';
            var expression = options['expression'](e);
            //var validation_state = eval(expression);
            if (!expression) {
                jQueryObjectBeforeErrorMessage = getjQueryObjectBeforeErrorMessage($(e));
                ...
                if (jQueryObjectBeforeErrorMessage.next('.' + options['error_message_class']).length == 0) {
					...
					$(e).parents("." + options['error_container_class']).addClass("error");
                }


                if (ValidationErrors[FormID].join("|").search($(e).attr('id')) == -1) 
                    ValidationErrors[FormID].push($(e).attr('id'));
                return false;
            }
            else {
                for (var i = 0; i < ValidationErrors[FormID].length; i++) {
                    if (ValidationErrors[FormID][i] == $(e).attr('id')) 
